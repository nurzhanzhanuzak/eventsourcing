...
* Added tracking recorder classes, to better support the development of arbitrary
  ...
  modules. Extracted from ProcessRecorder classes the functionality for recording
  tracking objects atomically with the recording of new state that is generated by
  processing events, and changed ProcessRecorder classes to inherit this functionality
  from the tracking recorders.
* Added subscribe() method to ApplicationRecorder, implemented in popo and postgres modules,
  allowing event-processing components to subscribe directly to an event store database,
  and thereby to be run independently of horizontally-scaled event-sourced applications.
...
* Adjusted domain module to ensure dataclasses.dataclass decorator is idempotent for aggregate
  and event classes, because processing a class twice with dataclass can cause problems when using
  fields with defaults and non-init fields. These problems can happen in the rare case when an Aggregate
  subclass is itself subclassed, and these aggregate classes use annotations and thereby depend
  on dataclass to define their __init__ methods and attributes, and these fields are specified
  with dataclass Field objects either to have a default value or to be excluded from the signature
...
* Moved methods and attributes specific to JSONTranscoder down from Transcoder class, eliminating
  "refused bequest" code smell for other subclasses of Transcoder. Adjusted signature of
